...
namespace OpenWifi {
	...
	*/
	bool RESTAPI_subscriber_devices_handler::Add_Device_Validate_Subscriber(AddDeviceContext &ctx) {
		...
		if (SDK::Prov::Signup::GetSignupDevice(nullptr, ctx.Mac)){
	...
	*/
	bool RESTAPI_subscriber_devices_handler::Add_Device_Gateway(AddDeviceContext &ctx) {
		...
		if (!SDK::Prov::Subscriber::CreateSubDeviceInfo(nullptr, ctx.InventoryTag, UserInfo_.userinfo, dev)) {
		...
		if (!SDK::Prov::Subscriber::GetDevice(nullptr, ctx.SubscriberInfo.accessPoints.list[0].macAddress, ctx.SubDevice)) {
	...
	*/
	bool RESTAPI_subscriber_devices_handler::Add_Device_Mesh(AddDeviceContext &ctx) {
		...
		if (!SDK::Prov::Subscriber::CreateSubDeviceInfo(nullptr, ctx.InventoryTag, UserInfo_.userinfo, ctx.SubDevice)) {
			Logger().error(fmt::format("Failed to create subdevice record for device: {} in owprov.", ctx.Mac));
			InternalError(RESTAPI::Errors::RecordNotCreated);
			return false;
		}
	...
	*/
	...
	bool RESTAPI_subscriber_devices_handler::Add_Device_to_Venue(AddDeviceContext &ctx,
		...
		*/
		bool RESTAPI_subscriber_devices_handler::Delete_Device_Update_Database(DeleteDeviceContext &
																			   ctx) {
			Logger().information(
		...
		*/
		void RESTAPI_subscriber_devices_handler::DoDelete() {
			...
			if (apList.front().macAddress == ctx.Mac) {
				...
				Logger().debug(fmt::format("Deleting mesh device: [{}] for subscriber: [{}].",
										   ctx.Mac, ctx.SubscriberInfo.id));
