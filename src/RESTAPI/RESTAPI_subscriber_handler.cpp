...
namespace OpenWifi {
	...
	bool RESTAPI_subscriber_handler::PrepareSubInfoObject(
		...
		StorageService()->SubInfoDB().CreateDefaultSubscriberInfo(UserInfo_, subInfo, devices);
		return true;
	}
	...
	bool RESTAPI_subscriber_handler::ProvisionSubscriber(SubObjects::SubscriberInfo &subInfo) {
		...
		if (provisionResponse && provisionResponse->has("boardId")) {
			try {
				subInfo.boardId = provisionResponse->getValue<std::string>("boardId");
				subInfo.modified = Utils::Now();
				StorageService()->SubInfoDB().UpdateRecord("id", subInfo.id, subInfo);
			} catch (const std::exception &e) {
				Logger().warning(fmt::format("Failed to parse boardId from provisioning response: {}.", e.what()));
			}
		}
		return true;
	}
