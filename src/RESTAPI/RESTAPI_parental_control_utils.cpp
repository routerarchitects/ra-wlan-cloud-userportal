...
namespace OpenWifi::RESTAPI::ParentalControl {
	...
	{
		Poco::JSON::Array::Ptr configRaw;
		if (!ExtractConfigRawSnapshot(mutationResponse, configRaw, configRawRequired)) {
			const auto context = invalidPayloadContext.empty() ? fmt::format("subscriber={} {}={}", subscriberId, objectType, applyTargetId) : invalidPayloadContext;
			logger.error(fmt::format("{}: invalid parental-control {} payload ({})", operationName, objectType, context));
			handler.InternalError(RESTAPI::Errors::InternalError);
			return false;
		}
		if (!configRaw) {
		...
		return HandleApplyConfigRawResult(
		    handler,
		    ApplyConfigRaw(handler, logger, subscriberId, operatorId,
		                  applyTargetId, configRaw, operationName, objectType));
	}

	void StripConfigRawFromMutationResponse(Poco::JSON::Object::Ptr mutationResponse) {
		if (mutationResponse && mutationResponse->has("config-raw")) {
			mutationResponse->remove("config-raw");
		}
	}
	                                         ...
	                                         const std::string &normalizeTimezone)
	{
		...
		if (!ApplyConfigRawFromMutationResponse(
		        handler, logger, subscriberId, operatorId, applyTargetId,
		        mutation.response, operationName, objectType, configRawRequired,
		...
		if (successResponse != MutationSuccessResponse::Ok && !response) {
			logger.error(fmt::format("{}: null parental-control {} response object", operationName, objectType));
		...
		case MutationSuccessResponse::ReturnObjectWithoutConfigRaw:
			StripConfigRawFromMutationResponse(response);
			return handler.ReturnObject(*response);
